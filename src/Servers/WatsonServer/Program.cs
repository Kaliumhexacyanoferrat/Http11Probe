...
var server = new Webserver(settings, async ctx =>
{
    ...
    if (ctx.Request.Url.RawWithoutQuery == "/echo")
    {
        var sb = new System.Text.StringBuilder();
        foreach (var key in ctx.Request.Headers.AllKeys)
            sb.AppendLine($"{key}: {ctx.Request.Headers[key]}");
        await ctx.Response.Send(sb.ToString());
    }
    else if (ctx.Request.Method == WatsonWebserver.Core.HttpMethod.POST && ctx.Request.Data != null)
    {
